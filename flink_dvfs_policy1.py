        ...
        if(interval!=-1):
            ...
            for vid in vertex_ids:
                ...
                for vtask in res['subtasks']:
                    ...
                    t_busytime,jt_busytime=get_task_metrics_details(job_id, vid, tid+'.busyTimeMsPerSecond')
                    t_backpressure,jt_backpressure=get_task_metrics_details(job_id, vid, tid+'.backPressuredTimeMsPerSecond')
                    t_idletime,jt_idletime=get_task_metrics_details(job_id, vid, tid+'.idleTimeMsPerSecond')
                    t_opsin,jt_opsin=get_task_metrics_details(job_id, vid, tid+'.numRecordsInPerSecond')
                    t_opsout,jt_opsout=get_task_metrics_details(job_id, vid, tid+'.numRecordsOutPerSecond')
