...
function BO {
	...
	for fr in $FLINK_RATE; do
		...
		echo "[INFO] Run Experiment"
 		echo "🟢 [INFO] python -u runBO.py --flinkrate ${fr} --bufftimeout -1 --nrepeat 0 --itr 1 --dvfs 1 --cores ${NCORES} --query ${MQUERY} --policy ${pol} --nsource ${nsrc} --nmapper ${nmapper} --nsink ${nsink} 🟢"
		cleanLogs
		ssh ${IPMAPPER} sudo systemctl stop rapl_log
		ssh ${IPMAPPER} sudo rm /tmp/rapl.log
		ssh ${IPMAPPER} sudo systemctl restart rapl_log
		sleep 1
		python -u runBO.py --flinkrate ${fr} --bufftimeout -1 --nrepeat 0 --itr 1 --dvfs 1 --cores ${NCORES} --query ${MQUERY} --policy ${pol} --nsource ${nsrc} --nmapper ${nmapper} --nsink ${nsink}
		sleep 1
		ssh ${IPMAPPER} sudo systemctl stop rapl_log
 		loc="./logs/${MQUERY}_cores${NCORES}_frate${fr}_fbuff-1_itr1_${pol}dvfs1_source${nsrc}_mapper${nmapper}_sink${nsink}_repeat0"
	...
	for i in `seq ${BEGIN_ITER} 1 $NITERS`; do
		...
		for pol in $MPOLICY; do
		    ...
		    ssh ${IPMAPPER} sudo systemctl stop rapl_log
 		    ...
 		    scp -r ${IPMAPPER}:/tmp/rapl.log ${loc}/rapl.log
		    scp -r $loc kd:/home/handong/sesadata/flink/imgproc_7_20_2024/
 		    echo "[INFO] FINISHED"
		done
	    done
	done    
