...
export NSOURCES=${NSOURCES:="16"}
export NMAPPERS=${NMAPPERS:="4 12"}
export NSINKS=${NSINKS:="16"}
...
export MCFG=${MCFG:="$(nproc);4;$(nproc) $(nproc);12;$(nproc)"} # Sources; Mappers; Sinks
...
function dynamic {        
	...
	# Doing a warmup run
	python -u runexperiment_cloudlab.py --flinkrate ${fr} --bufftimeout -1 --itr 1 --dvfs 1 --nrepeat 0 --cores ${NCORES} --query ${MQUERY} --policy "ondemand" --nsource ${nsrc} --nmapper ${nmapper} --nsink ${nsink}
...
function dynamicPin {
	...
	for i in `seq ${BEGIN_ITER} 1 $NITERS`; do
		...
		for pol in $MPOLICY; do
		    ...
		    echo $fc
		    ssh ${IPMAPPER} "cd ~/experiment-scripts && FLINKC=${fc} CS='C1 C2 C3 C6' ./pin_experiments.sh run" &
